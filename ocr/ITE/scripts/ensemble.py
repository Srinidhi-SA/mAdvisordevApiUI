...
def add_azure_conf(final_json, analysis):  ## ADDS AZURE CONFIDENCE KEY
    ...
    for i,line in enumerate(analysis["lines"]):
        wc[i + 1] = [(word['boundingBox'], word['text'], word['confidence'],
                      line['appearance']['style']) if 'appearance' in line.keys()
                     else (word['boundingBox'], word['text'], word['confidence'], 'print') for word in line['words']]
